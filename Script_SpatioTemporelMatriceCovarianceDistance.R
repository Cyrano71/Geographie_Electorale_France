#SpatioTemporelMatriceCovarianceDistance
...
model.path <- paste0(getwd(),"/models/ModelSpatioTemporelMatriceCovarianceDistance.bug")
...
vendee.sf$PPL1 <- sqrt(colMeans(samples$sims.list$PPL[,,1])) 

library(tmap)

jpeg("ppl\\SpatioTemporelMatriceCovarianceDistancePPL.jpg", width = 850, height = 850)

tmap_mode('plot') + tm_shape(vendee.sf) + 
tm_polygons('PPL1', title = "PPL", palette ="Oranges", breaks = c(0, 20, 40, 60, 80, 100, 130, 200, 300, 400, Inf)) 
